...
$$
我们观察$(F(x)-F_0(x))^2$，由定义可知，$F(x)$和$F_0(x)$前$\frac{n}{2}-1$项是相同的那么$F(x)-F_0(x)$第一个非零之处是第$\frac{n}{2}$项，而平方一下就是第$n$项（因为上取整，所以也有可能是第$n+1$项），此时我们发现，因为是在$mod\quad x^n$意义下进行，所以这一项是没有用的。也就是说，在二阶导及其后面的所有项在$mod\quad x^n$下都是$0$，可以直接省略！
